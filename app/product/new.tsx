...
export default function NewProductScreen() {
  ...
  const pickPhoto = async () => {
    ...
    if (status !== 'granted') { showToast('Photo library access required', 'error'); return; }
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          {photoUri ? (
            ...
            <View style={styles.photoButtons}>
              <Pressable onPress={takePhoto} style={[styles.photoBtn, { backgroundColor: theme.backgroundTertiary, borderColor: theme.cardBorder }]}>
                ...
                <Text style={[styles.photoBtnText, { color: theme.textSecondary }]}>Camera</Text>
              </Pressable>
              <Pressable onPress={pickPhoto} style={[styles.photoBtn, { backgroundColor: theme.backgroundTertiary, borderColor: theme.cardBorder }]}>
                ...
                <Text style={[styles.photoBtnText, { color: theme.textSecondary }]}>Gallery</Text>
              </Pressable>
            </View>
          )}
        </View>

        <FieldGroup title="Basic Info" theme={theme}>
          <Field label="Product Name *" theme={theme}>
            <TextInput style={[styles.input, { backgroundColor: theme.backgroundTertiary, borderColor: theme.cardBorder, color: theme.text }]} value={form.name} onChangeText={(v) => set('name', v)} placeholder="Enter product name" placeholderTextColor={theme.placeholder} />
          </Field>
          <Field label="Brand" theme={theme}>
            <TextInput style={[styles.input, { backgroundColor: theme.backgroundTertiary, borderColor: theme.cardBorder, color: theme.text }]} value={form.brand} onChangeText={(v) => set('brand', v)} placeholder="Brand name" placeholderTextColor={theme.placeholder} />
          </Field>
          <Field label="Serial Number / SKU" theme={theme}>
            <TextInput style={[styles.input, { backgroundColor: theme.backgroundTertiary, borderColor: theme.cardBorder, color: theme.text }]} value={form.serialNumber} onChangeText={(v) => set('serialNumber', v)} placeholder="Serial or SKU" placeholderTextColor={theme.placeholder} autoCapitalize="characters" />
          </Field>
          <Field label="Barcode / EAN" theme={theme}>
            <TextInput style={[styles.input, { backgroundColor: theme.backgroundTertiary, borderColor: theme.cardBorder, color: theme.text }]} value={form.barcode} onChangeText={(v) => set('barcode', v)} placeholder="Barcode number" placeholderTextColor={theme.placeholder} keyboardType="number-pad" />
          </Field>
        </FieldGroup>

        <FieldGroup title="Category" theme={theme}>
          <Pressable
        ...
        <FieldGroup title="Pricing" theme={theme}>
        ...
        <FieldGroup title="Stock" theme={theme}>
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Text style={[styles.fieldLabel, { color: theme.textSecondary }]}>Initial Quantity</Text>
        ...
        <FieldGroup title="Additional Info" theme={theme}>
          <Field label="Supplier" theme={theme}>
            <TextInput style={[styles.input, { backgroundColor: theme.backgroundTertiary, borderColor: theme.cardBorder, color: theme.text }]} value={form.supplier} onChangeText={(v) => set('supplier', v)} placeholder="Supplier name" placeholderTextColor={theme.placeholder} />
          </Field>
